...
import { parseIntelHex, type ParsedFirmware } from '../protocol/hex-parser.js';
import { BootloaderClient, type FlashProgress } from '../protocol/bootloader-client.js';

let selectedFirmware: ParsedFirmware | null = null;

export function renderFirmwareTab(container: HTMLElement, state: AppState): void {
  container.innerHTML = `
    <div class="card">
...
function bindFirmwareActions(container: HTMLElement, state: AppState): void {
  const fileEl = container.querySelector<HTMLInputElement>('#fw-file')!;
  const metaEl = container.querySelector<HTMLElement>('#fw-meta')!;
  const flashBtn = container.querySelector<HTMLButtonElement>('#fw-flash')!;
  const verifyEl = container.querySelector<HTMLInputElement>('#fw-verify')!;
  const progressEl = container.querySelector<HTMLProgressElement>('#fw-progress')!;
  const progressText = container.querySelector<HTMLElement>('#fw-progress-text')!;
  const logEl = container.querySelector<HTMLElement>('#fw-log')!;
  ...
  const setProgress = (p: FlashProgress): void => {
  ...
  flashBtn.addEventListener('click', async () => {
    ...
    let boot: BootloaderClient | null = null;
    try {
