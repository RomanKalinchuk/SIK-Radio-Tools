...
import {
  ...
  sanitizeParams,
  type ParamValues,
} from '../params/mapper.js';
import { getSikClient, setCurrentParams } from './app.js';
import { showToast } from './toast.js';
let localParams: ParamValues = {};

export function renderSettingsTab(container: HTMLElement, state: AppState): void {
...
function renderParamGrid(gridEl: HTMLElement, params?: ParamValues): void {
  ...
  gridEl.querySelectorAll('input, select').forEach((el) => {
    el.addEventListener('change', () => {
      const key = (el as HTMLElement).dataset.param!;
...
function applyParamsToGrid(params: ParamValues): void {
  for (const def of SIK_PARAM_SCHEMA) {
    ...
    const el = document.querySelector<HTMLInputElement | HTMLSelectElement>(
      `input[data-param="${def.key}"], select[data-param="${def.key}"]`
    );
    if (el) {
      el.value = String(val);
...
function bindSettingsActions(container: HTMLElement, _state: AppState): void {
  container.querySelector('#btn-load')?.addEventListener('click', async () => {
    ...
    const loadBtn = container.querySelector('#btn-load') as HTMLButtonElement;
    ...
    try {
      ...
      if (rawCount === 0) throw new Error('Radio returned no parameters');

      localParams = { ...getDefaultParams(), ...loaded };
      setCurrentParams(localParams);

      const gridEl = container.querySelector<HTMLElement>('#param-grid');
      if (gridEl) applyParamsToGrid(localParams);
      ...
      if (/timeout|did not respond/i.test(msg)) {
        showToast(
          'error',
          'Radio did not respond. Enter command mode first: Terminal → "Enter Cmd Mode", then Load from Radio again.'
        );
      } else {
        showToast('error', msg);
      }
