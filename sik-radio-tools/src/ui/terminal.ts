...
const MAX_LINES = 500;
const terminalLines: Array<{ type: 'tx' | 'rx'; text: string; time: string }> = [];
let lineListenerUnsubscribe: (() => void) | null = null;
...
function appendLine(type: 'tx' | 'rx', text: string): void {
...
export function renderTerminalTab(container: HTMLElement, _state: AppState): void {
  ...
  document.querySelectorAll('[data-cmd]').forEach((btn) => {
    btn.addEventListener('click', () => {
      sendCommand((btn as HTMLElement).dataset.cmd ?? '');
    });
  });
  ...
  if (transport?.isConnected && transport.addLineListener) {
    lineListenerUnsubscribe = transport.addLineListener((line) => {
      if (line.trim()) appendLine('rx', line.trim());
    });
  }
}
