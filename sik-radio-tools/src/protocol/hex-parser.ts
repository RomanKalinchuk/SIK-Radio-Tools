...
export function parseIntelHex(content: string): ParsedFirmware {
  ...
  for (const line of lines) {
    ...
    if (type === 0x00) {
      ...
      if (upper !== 0) usesBanking = true;
      ranges.set(abs, data);
    } else if (type === 0x04) {
      if (count !== 2) throw new Error('Invalid type 04 record');
  ...
  const segments: FirmwareSegment[] = [...merged.entries()]
    .sort((a, b) => a[0] - b[0])
