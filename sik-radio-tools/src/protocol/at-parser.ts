...
export function parseATI5Response(lines: string[]): Record<string, string | number> {
...
export function isOK(line: string): boolean {
...
export function parseATResponse(lines: string[]): ATParseResult {
  const result: ATParseResult = { ok: false, lines: [] };
  for (const line of lines) {
    const t = line.trim();
    if (isOK(t)) {
      result.ok = true;
