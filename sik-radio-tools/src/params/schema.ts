...
export type ParamCategory = 'basic' | 'advanced';
...
export interface ParamDef {
  ...
  /** Optional S-register mapping (e.g. S1, S2) */
  register?: string;
}

/** Common SiK parameters - 900MHz defaults where applicable */
export const SIK_PARAM_SCHEMA: ParamDef[] = [
  {
    key: 'SERIAL_SPEED',
    label: 'Serial Speed',
...
export function keyToRegister(key: string): string | undefined {
  return SIK_PARAM_SCHEMA.find((p) => p.key === key)?.register;
}

/** Map S-register to param key */
export function registerToKey(reg: string): string | undefined {
